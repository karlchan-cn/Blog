 ...
 * 
 */
public static PageRequest extractRequstParameters(final HttpServletRequest request) {
	final PageRequest pageRequest = new PageRequestImpl();
	pageRequest.setPageNumber(Integer.valueOf(request.getParameter(CONST_PARANAME_iDisplayStart)));
	pageRequest.setPageSize(Integer.valueOf(request.getParameter(CONST_PARANAME_iDisplayLength)));
	return pageRequest;
 ...
 * @return 表格数据.
 */
public static JSONObject assembleTableData() {
	return null;
}
}
